...
def api_location(request, pk):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if count > 0:
                deleteLocationVOurl = f'http://hats-api:8000/api/locations/{pk}/'
                requests.delete(deleteLocationVOurl)
            return JsonResponse(
                location,
                encoder=LocationEncoder,
                safe=False,
...
def api_bin(request, pk):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            count, _ = bin.delete()
            if count > 0:
                deleteBinVOurl = f'http://shoes-api:8000/api/bins/{pk}/'
                requests.delete(deleteBinVOurl)
            return JsonResponse(
                bin,
                encoder=BinEncoder,
                safe=False,
